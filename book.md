...
| 中级：已经能用 Agent 写功能 | 解决需求漂移、弱测试、疑难 bug 和跨会话失真 | 3 → 4 → 5 → 7 → 8 → 13 | 让 spec、ticket、测试和 handoff 组成连续工程链 |
| 高阶：正在带团队或建 Harness | 设计可组合、可审计、可降级的 Agent 工作流 | 6 → 9 → 11 → 12 → 13 → 14 | 把 Skill 改写成项目级流程，并用证据边界约束完成声明 |
...
| 想看 Codex、Claude Code、Zcode 会话提炼出的流程细节 | 第 13 章 |
...
| 脱敏会话模式 | 一种流程在真实 Agent 会话中曾经出现 | 某个私有项目已经这样实现，更不是成功率 |
