...
正文解释“为什么”，实践把它变成可复跑的结果。三层的作用不同：

| 层级 | 时长 | 用途 | 是否阻塞后续 |
|---|---:|---|---|
| 微练习 | 5–10 分钟 | 用每章验收问题检查自己是否理解一个概念 | 否 |
| 实践关卡 | 25–45 分钟 | 用 3–5 个动作完成一项单一工程能力 | 是：缺少产物就先补齐 |
| 综合交付 | 60–150 分钟 | 串联规格、反馈、review 与 handoff，完成小型垂直切片 | 最后进行 |

微练习可以在读完任何一节后马上完成；它不要求固定技术栈，也不把思考题伪装成完成证据。

### 0.1.2 正文和实践路径是一条阅读路径

[实践路径](#practice-readme)嵌在正文的关键转折之后，同时保留独立链接供需要分享某一关的人使用。六个关卡和一个综合交付都要求在自己的仓库留下“做过什么、怎样验证、下一步怎样交接”：

| 关卡 | 先练什么 | 完成时必须留下什么 |
|---|---|---|
| 实践关卡 1 | 事实调查与人类决策分界 | 决策记录 + 最小验证命令 |
| 实践关卡 2 | 决定如何变成 spec 与 ticket | 最小 spec + 第一张垂直 ticket |
| 实践关卡 3 | bug 怎样从猜测变成反馈环 | 稳定失败的目标命令 |
| 实践关卡 4 | 用最小修复把同一条命令从红变绿 | 最小 diff + 绿色验证 |
| 实践关卡 5 | 分开工程标准和用户承诺两条 review 轴 | 两份独立 review 结论 |
| 实践关卡 6 | 交接前先由新会话复述 | 可执行 handoff |
| 综合交付 | 把前面的能力串成一条小型垂直切片 | spec、验证、review 与 handoff |

第一次读本书时，可以按“读第 3 章 → 做实践关卡 1 → 读第 4 章 → 做实践关卡 2”的节奏前进。任何一关卡住，不要跳到更复杂的 Skill；先修复该关缺失的事实、决定或验证。
...
| 想把阅读变成一次可验证的实操 | [实践路径](#practice-readme) |
...
#### [实践关卡 1 · 先找到答案](#practice-01-find-the-answer)

把环境事实、人类决策和范围结论分开。产物：决策记录 + 最小验证命令。

#### [实践关卡 2 · 把决定写成可施工规格](#practice-02-decision-to-spec)

把已确认的决定压缩为 spec 与第一张垂直 ticket。产物：spec + tracer-bullet ticket。

#### [实践关卡 3 · 让一个 bug 先变红](#practice-03-red-green-bug)

把症状变成一条穿过公共 seam 的失败命令。产物：红色结果 + 可重复症状。

#### [实践关卡 4 · 用最小修复闭合证据](#practice-04-minimal-fix-evidence)

用同一条命令从红到绿，并固化回归。产物：最小 diff + 红绿证据 + 回归检查。

#### [实践关卡 5 · 分开两条 review 轴](#practice-05-two-axis-review)

分别审查工程标准与用户承诺。产物：Standards 与 Spec 两份结论。

#### [实践关卡 6 · 交接前先复述](#practice-06-handoff-rehearsal)

让新会话只读交接包后复述状态与唯一下一步。产物：可验证 handoff。

### [综合交付 · 完成一次可交接交付](#practice-07-capstone-delivery)
...
[进入实践关卡 2](#practice-02-decision-to-spec) · [返回实践路径](#practice-readme)
...
[进入实践关卡 3](#practice-03-red-green-bug) · [返回实践路径](#practice-readme)
...
[进入实践关卡 4](#practice-04-minimal-fix-evidence) · [返回实践路径](#practice-readme)
...
[进入实践关卡 5](#practice-05-two-axis-review) · [返回实践路径](#practice-readme)
...
[进入实践关卡 6](#practice-06-handoff-rehearsal) · [返回实践路径](#practice-readme)
...
[进入综合交付](#practice-07-capstone-delivery) · [返回实践路径](#practice-readme)
...
[返回实践路径](#practice-readme) · [回到全书](#top)
